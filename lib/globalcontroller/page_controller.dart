...
//     if (page is BusSearchResultScreen || page is SeatSelectionScreen) {
...
class Mypagecontroller extends GetxController {
  ...
  /// 🔹 Page → Background color mapper
  Color _getBackgroundColorForPage(Widget page) {
    if (page is BusSearchResultScreen || page is SeatSelectionScreen) {
      return const Color(0xFF2E2982);
    }
  ...
  /// 🔹 Change page handler
  void changePage(Widget page, {bool isMainPage = true}) {
    backgroundColor.value = _getBackgroundColorForPage(page);
  ...
  bool goBack() {
    if (pageStack.length > 1) {
      ...
      final Widget currentPage = pageStack.last;
      backgroundColor.value = _getBackgroundColorForPage(currentPage);
